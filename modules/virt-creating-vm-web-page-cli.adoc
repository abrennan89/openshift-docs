...
+
`name` (under `metadata`):: The VM name.
`name` (under `dataVolumeTemplates.metadata`):: The data volume name.
`url`:: The URL of the image.
`storage`:: The size of the storage requested for the data volume.
`name` (under `instancetype`):: The instance type to use to control resource sizing of the VM.
`name` (under `preference`):: The preference to use.
