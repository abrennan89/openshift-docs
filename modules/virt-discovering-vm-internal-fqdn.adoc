...
where:
+
`expose`:: Specifies a label that must match the `spec.selector` attribute of the `Service` manifest that you previously created.
`hostname`:: Specifies the hostname. If this attribute is not specified, the resulting DNS A record takes the form of `<vm.metadata.name>.<vm.spec.subdomain>.<vm.metadata.namespace>.svc.cluster.local`.
`subdomain`:: Specifies the subdomain. The `spec.subdomain` attribute must match the `metadata.name` value of the `Service` object.
