...
`nodeSelector`:: Optional: If you do not include the `nodeSelector` parameter, the policy applies to all nodes in the cluster. This example uses the `node-role.kubernetes.io/worker: ""` node selector to select all worker nodes in the cluster.
`eth1-nic`:: Specifies the reference to the node NIC to which the bridge attaches.
`type`:: Specifies the type of interface. This example creates a bridge.
`ipv4`:: Specifies the IP address of the bridge interface. This value matches the IP address of the NIC which is referenced by the `spec.capture.eth1-nic` entry.
