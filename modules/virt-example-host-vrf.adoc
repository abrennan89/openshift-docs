...
`vrf`:: Specifies a node selector. This example applies the policy to all nodes with the label `vrf:true`.
`name` (under `interfaces`):: Specifies the name of the interface.
`type`:: Specifies the type of interface. This example creates a VRF instance.
`port`:: Specifies the node interface to which the VRF attaches.
`route-table-id`:: Specifies the route table ID for the VRF.
