...
You can configure permissions for {product-title} users by binding the `batch-user` cluster role to a user or group of users.
...
<4> Add details about the `batch-user` cluster role.
...
* You can verify that the `RoleBinding` object was applied correctly by running the following command and verifying that the output contains the correct information for the `batch-user` cluster role:
+
[source,yaml]
----
