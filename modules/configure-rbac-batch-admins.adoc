...
You can configure permissions for batch administrators by binding the `kueue-admin` cluster role to a user or group of users.
...
<3> Add details about the `kueue-admin` cluster role.
...
* You can verify that the `ClusterRoleBinding` object was applied correctly by running the following command and verifying that the output contains the correct information for the `kueue-admin` cluster role:
+
[source,yaml]
----
$ oc describe clusterrolebinding.rbac
----
// TODO: Ask eng/QE for an example output for this verification
