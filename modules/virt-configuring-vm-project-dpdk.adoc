...
+
`networkNamespace`:: The namespace where the `NetworkAttachmentDefinition` object is deployed.
`resourceName`:: The value of the `spec.resourceName` attribute of the `SriovNetworkNodePolicy` object that was created when configuring the cluster for DPDK workloads.
