...
+
`name`:: The VM name.
`name` (under `pvc`):: The name of the source PVC.
`namespace`:: The namespace of the source PVC.
`inferFromVolume` (under `instancetype`):: If the PVC source has appropriate labels, the instance type is inferred from the selected `DataSource` object.
`inferFromVolume` (under `preference`):: If the PVC source has appropriate labels, the preference is inferred from the selected `DataSource` object.
