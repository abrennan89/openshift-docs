...
where:
+
`name`:: Specifies the name of the node network configuration policy.
`ethernets-lldp`:: Specifies that LLDP is enabled for all ethernet ports that have the interface state set to `up`.
