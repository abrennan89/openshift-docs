...
{product-title} supports all-or-nothing semantics. This means that either an entire change is applied successfully to pods, or the entire change is rolled back, preventing partial updates and issues that could be caused by them.
...
. Batch administrators create and configure `ResourceFlavor`, `LocalQueue`, `ClusterQueue`, `AdmissionCheck`, and `BorrowingCohort` resources.
